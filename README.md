...
| [00b - 設計階段的需求考量](00b-設計階段的需求考量.md) | 設計前的決策條件與框架決策（自建 / 託管、實體 / 雲端、發行版家族、單向門清單、加權評分、決策關卡）、十個需求問題、需求 → 決策推導表（Tier / RPO / 生命週期 / 合規 / 工作負載 / 部署位置）、SLA / SLO / SLI 與可用性計算（串聯相乘、並聯冗餘、MTBF/MTTR、錯誤預算）、容量規劃與隱性用量、環境與網路規劃、維運模型、設計決策紀錄範本與上線前檢查清單 |
