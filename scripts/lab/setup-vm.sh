...
# 映像放 libvirt 的儲存目錄：qemu 使用者（uid qemu）要能讀寫，放家目錄會遇到 "Cannot access storage file ... Permission denied"
WORK=${LAB_VM_DIR:-/var/lib/libvirt/images/lab-vm}
[[ -d $WORK && -w $WORK ]] || sudo install -d -m 775 -o "$USER" -g "$(id -gn)" "$WORK"
...
ensure_libvirt() {
  # 🔵 Fedora 的 libvirt 是模組化 daemon（virtqemud 等），裝完預設沒啟動；🟠 Ubuntu 的 libvirtd 通常已啟動
  if ! virsh --connect qemu:///system uri >/dev/null 2>&1; then
    echo "==> 啟動 libvirt daemon（需要 sudo）"
    if systemctl list-unit-files virtqemud.socket >/dev/null 2>&1 && systemctl list-unit-files | grep -q '^virtqemud.socket'; then
      sudo systemctl enable --now virtqemud.socket virtnetworkd.socket virtstoraged.socket virtnodedevd.socket virtlogd.socket virtlockd.socket >/dev/null
    else
      sudo systemctl enable --now libvirtd >/dev/null
    fi
  fi
  if ! id -nG | grep -qw libvirt; then
    echo "==> 把 $USER 加入 libvirt 群組（重新登入後生效；本次先以 sg libvirt 執行）"
    sudo usermod -aG libvirt "$USER"
    exec sg libvirt -c "bash '$0' $*"
  fi
  virsh --connect qemu:///system net-info default >/dev/null 2>&1 || { echo "找不到 libvirt default 網路（virsh net-define /usr/share/libvirt/networks/default.xml）" >&2; exit 1; }
  virsh --connect qemu:///system net-list --name | grep -qx default || sudo virsh net-start default >/dev/null
