set -e
. /etc/os-release
if [ "$ID" = ubuntu ]; then apt-get install -y -q restic >/dev/null 2>&1; else dnf -y -q install restic >/dev/null 2>&1; fi
restic version
install -m 700 /root/backup-restic.sh /usr/local/bin/backup-restic.sh
...
echo "--- timer units"; cp /root/ex/restic-backup.service /root/ex/restic-backup.timer /etc/systemd/system/ 2>/dev/null || true; systemctl daemon-reload; systemd-analyze verify /etc/systemd/system/restic-backup.timer 2>&1 | head -2; systemctl enable --now restic-backup.timer 2>&1 | grep -v Created; systemctl list-timers restic-backup.timer --no-pager | sed -n 2p
